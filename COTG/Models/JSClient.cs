...
namespace CnV
{
	...
	/// The j s client.
	/// </summary>
	public class JSClient:CnVServer
	{
		...
		public static void JSInvoke(string func)
		{
			AppS.DispatchOnUIThreadLow(async () =>
			{
				...
				try
				{
					await view.ExecuteScriptAsync(func);
		...
		public static async Task JSInvokeTask(string func)
		{

			// this won't await the actually js call
			 await AppS.DispatchOnUIThreadTask(async () => await view.ExecuteScriptAsync(func));
		...
		public static JSVars PlayerVars(int pid)
		{
			if(pid == -1)
				return jsVars;
			for(var i = 0; i < jsVarsByPlayer.Length; ++i)
			{
		...
		{
			var t0 = AppS.GetAppString("JS/funky");
			var t1 = AppS.GetAppString("JS/DHRUVCC.js");
			var t2 = AppS.GetAppString("JS/J0EE");
		...
		private static async void WebViewException(Exception ex)
		{
			...
			if(await AppS.DoYesNoBox("Some is wrong with Webview", "Download the webview?", cancel: null) == 1)
...
//			throw new NotImplementedException();
			...
			CoreWebView2ScriptDialogOpeningEventArgs args)
		{
			...
			try
			{
				var rv = await AppS.DoYesNoBox(args.DefaultText, args.Message, args.Kind.ToString(), null, null);
		...
		private static async void View_WebResourceRequested(CoreWebView sender, CoreWebView2WebResourceRequestedEventArgs args)
		{
			...
			try
			{
				...
				if(userAgent.IsNullOrEmpty())
				{
					...
					if(!userAgent.IsNullOrEmpty())
						AppS.QueueOnUIThread(() => UserAgent.SetUserAgent(userAgent));
				}
		...
		{
			AppS.DispatchOnUIThreadLow(() => coreWebView.PostWebMessageAsString($"{{\"postMouseEvent\":{{\"x\":{x},\"y\":{y},\"eventName\":\"{eventName}\",\"button\":{button},\"dx\":{dx},\"dy\":{dy}}}}}"));
		}
		...
		{
			AppS.DispatchOnUIThreadLow(() => ExecuteScriptAsync("setStayAlive", stayAlive ? 1 : 0));
		}
		public static void SendChat(int channel, string message)
		{
			try
			{
				...
				for(; ; )
				{
					...
					Task.Delay(1000).ContinueWith((_) => AppS.DispatchOnUIThreadLow(() => SendChat(channel, remainder)));
					break;
				}
			}
			catch(Exception e)
		...
		{
			Note.Show($"{Get(cid).nameMarkdown} Clear Center Res");
				...
				if(!councillorsChecked)
					if(jse.TryGetProperty("cob", out var cob))
					{
						...
						foreach(var c in cob.EnumerateObject())
						{
							...
							if(t <= 0)
							{
								AppS.DispatchOnUIThreadLow(ShowCouncillorsMissingDialog);
								break;
							}
	...
	if(citySwitch != 0)
		...
		public static async Task CityRefresh()
		{
			...
			{
				// don't wait on this
				await JSInvokeTask($"cityRefresh({Spot.build})");
					...
					try
					{
						...
						//	for (int i = 0; i < clientCount; ++i)
						{
							...
							foreach(var s in cookies.Split(';', StringSplitOptions.RemoveEmptyEntries|StringSplitOptions.TrimEntries))
							{
								...
								if(vEq== -1)
									continue;
								cookieContainer.Add(new Cookie(s.Substring(0, vEq).Trim(), s.Substring(vEq+1).Trim(), "/", "crownofthegods.com"));
							}

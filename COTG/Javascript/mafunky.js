// ==UserScript==
// @name Cotg MAfunky
// @namespace https://github.com/dwuLive/Mfunky
// @version 1.0.4
...
// @grant none
// @updateURL https://github.com/dwulive/Mfunky/raw/master/Mafunky.user.js
// @downloadURL https://github.com/dwulive/Mfunky/raw/master/Mafunky.user.js
// ==/UserScript==
function LocalStoreAsInt(__s, __def = 0) {
    var rv = localStorage.getItem(__s);
    if (rv == null)
        return __def;
    return parseInt(rv);
}
...
class Coord extends Number {
    constructor(_a) { super(_a); }
    get cid() { return this.valueOf(); }
    get x() {
        return this.valueOf() & 65535;
    }
    get y() {
...
function OverviewPost(url, post, onSuccess = null) {
    var headers = new Headers({
        ...
        'pp-ss': '0',
        "Referer": 'https://w19.crownofthegods.com/overview/overview.php?s=0' // may be redundant
    ...
    fetch(new Request(url, {
        ...
        headers: headers,
        //  redirect: 'follow',
        referrer: 'https://w19.crownofthegods.com/overview/overview.php?s=0',
        mode: 'cors',
        cache: "no-cache",
...
//			}
//		}
//		minddate_=new Date(minddate_.getTime()-Number(localStorage.getItem("retHr"))*60*60*10000);
//		/** @type {string} */
...
//			});
//		},100);
//		$("#reinforcetimingselect").val(Number(commandInfo.dep)+1).change();
...
//			commandInfo.total.push(Math.ceil(total*Number($("#perc").val())/100));
//			commandInfo.home.push(Math.ceil(troops[i].home*Number($("#perc").val())/100));
...
function updateattack_() {
    ...
    var troops = cotg.city.troops();
    for (let id in cotg.city.troops()) {
        var t = troops[id];
        if (t.total == 0)
            continue;
        var i = TroopNameToId(id);
...
function incomings_() {
    ...
    $("#iaBody tr").each(function () {
        let tid_2 = GetIntData($(":nth-child(5)", this).children().children());
        let sid_ = GetIntData($(":nth-child(10)", this).children());
        /** @type {number} */
        var tx_ = tid_2 % 65536;
        /** @type {number} */
        var sx_1 = sid_ % 65536;
        ...
        /** @type {number} */
        var mdiff_ = parseInt(atime_.substring(3, 5)) - parseInt(stime_.substring(3, 5));
        /** @type {number} */
        var sdiff_ = parseInt(atime_.substring(6, 8)) - parseInt(stime_.substring(6, 8));
        /** @type {!Date} */
        var d_3 = new Date;
        ...
        var travelingts_ = TwoDigitNum(locks_);
        var travelingtm_ = TwoDigitNum(lockm_);
        var travelingth_ = TwoDigitNum(lockh_);
        /** @type {number} */
...
function getbossinfo_() {
    ...
    for (i_19 in wdata_.bosses) {
        ...
        /** @type {number} */
        var templvl_ = Number(wb.substr(1, 2)) - 10;
        /** @type {number} */
        var tempy_3 = Number(wb.substr(4, 3)) - 100;
        /** @type {number} */
        var tempx_3 = Number(wb.substr(7, 3)) - 100;
        /** @type {number} */
...
/*function openbosswin_() {
    ...
    for(i_20 in bossinfo_.cont) {
        */ /** @type {number} */ /*
var distance_=Math.sqrt((bossinfo_.x[i_20]-cdata_.x)*(bossinfo_.x[i_20]-cdata_.x)+(bossinfo_.y[i_20]-cdata_.y)*(bossinfo_.y[i_20]-cdata_.y));
if((cdata_.th[2]||cdata_.th[3]||cdata_.th[4]||cdata_.th[5]||cdata_.th[6]||cdata_.th[8]||cdata_.th[9]||cdata_.th[10]||cdata_.th[11])&&cdata_.th[14]==0) {
    if(bossinfo_.cont[i_20]==cdata_.cont) {
        if(cdata_.th[2]||cdata_.th[3]||cdata_.th[4]||cdata_.th[5]||cdata_.th[6]) {
            */ /** @type {number} */ /*
var minutes_=distance_*ttspeed_[2]/ttspeedres_[2];
*/ /** @type {string} */ /*
var time_5=`${Math.floor(minutes_/60)}h ${Math.floor(minutes_%60)}m`;
}
if(cdata_.th[8]||cdata_.th[9]||cdata_.th[10]||cdata_.th[11]) {
*/ /** @type {number} */ /*
...
bosslist_.distance.push(RoundTo2Digits(distance_));
bosslist_.time.push(time_5);
bosslist_.cont.push(bossinfo_.cont[i_20]);
}
}
if(distance_<180) {
if(cdata_.th[14]||cdata_.th[15]||cdata_.th[16]) {
*/ /** @type {number} */ /*
...
for(i_20 in cdata_.th) {
    */ /** @type {!Array} */ /*
var notallow_=[0,1,7,12,13];
if(notallow_.indexOf(i_20)==-1) {
    if(cdata_.th[i_20]>0) {
        */ /** @type {string} */ /*
idle_=`${idle_}<td><div class='${tpicdiv_[i_20]}' style='text-align: right;'></div></td><td style='text-align: left;'><span id='thbr${i_20}' style='text-align: left;'>${cdata_.th[i_20]}</span></td>`;
}
}
}
*/ /** @type {string} */ /*
...
function recallraidl100_() {
    ...
    var i_21 = 0;
    var x_75;
    for (x_75 in cdata_.th) {
        /** @type {number} */
        total_ = Number(cdata_.th[x_75]);
        /** @type {number} */
...
function carrycheck_() {
    ...
    var i_22 = 0;
    var x_76;
    for (x_76 in cdata_.th) {
        /** @type {number} */
        total_1 = Number(cdata_.th[x_76]);
        /** @type {number} */
...
function GetCarry() {
    return LocalStoreAsInt('carry', 1.02);
}
var count_14;
/**
...
function carry_percentage_(total_loot_1) {
    ...
    /** @type {!Array} */
    var km_ = Number[0];
    for (let x_77 in cdata_.th) {
        /** @type {number} */
        var home_ = Number(cdata_.th[x_77]);
        /** @type {number} */
...
function PostMMNIO(j_12) {
    ...
    for (let i_25 in hubs_.cid) {
        ...
        hubs_.distance.push(Distance(_cid.x, _cid.y, cdata_.x, cdata_.y));
    }
    /** @type {number} */
...
function setinfantry_() {
    ...
    for (i_26 in hubs_1.cid) {
        ...
        hubs_1.distance.push(Math.sqrt((tempx_5 - cdata_.x) * (tempx_5 - cdata_.x) + (tempy_5 - cdata_.y) * (tempy_5 - cdata_.y)));
    }
    /** @type {number} */
...
var __a6 = {
    ...
    ccazzx: { encrypt: null, decrypt: null },
};
...
function updateres_(data_45) {
    ...
    $.each(data_45, function () {
        var cid_3 = this.id;
        /** @type {number} */
        var __x = Number(cid_3 % 65536);
        /** @type {number} */
        var __y = Number((cid_3 - __x) / 65536);
...
function GetContinent(__x, __y) {
    return Math.floor(__x / 100) + Math.floor(__y / 100) * 10;
}
function SendDonation() {
    ...
    gamePost(req, {
        cid: _cid, f: __a6.encryptJs(req, args)
    });
    $(this).remove();
    //	cid: 17695065
...
function UpdateDonate(resData, blessData, filter) {
    ...
    $.each(resData, function () {
        let cid_3 = Number(this.id);
        if (filter != null && filter.indexOf(cid_3) == -1) {
            return;
        }
        /** @type {number} */
        let __x = Number(cid_3 % 65536);
        /** @type {number} */
        let __y = Number((cid_3 - __x) / 65536);
        ...
        for (let i = 0; i < blessData.a.length; ++i) {
            ...
            /** @type {number} */
            let tempx = Number(bcid % 65536);
            /** @type {number} */
            let tempy = Number((bcid - tempx) / 65536);
...
/** @type {!Array} */
var ttname_ = ["guard", "ballista", "ranger", "triari", "priestess", "vanquisher", "sorcerers", "scout", "arbalist", "praetor", "horseman", "druid", "ram", "scorpion", "galley", "stinger", "warship", "senator"];
/** @type {!Array} */
var layoutsl_ = [""];
/** @type {!Array} */
var layoutsw_ = [""];
...
class DoneWrapper {
    ...
    //constructor(_this: DoneWrapperC,data: any,textStatus: JQuery.Ajax.SuccessTextStatus,jqXHR: JQuery.jqXHR): void { };
    then(a) { return (a.text()); }
    catch(e) { return null; }
    finally(e) { return null; }
    inally(b) { return null; }
    done(cb) {
        this.req.then((data) => data.text()).then((_data) => {
            setTimeout(() => __avatarAjaxDone(this.url, _data), 100);
            //return cb(data);
            //		_this.req.
            //then(cb).
...
function avatarAjax(settings) {
    try {
        let headers = new Headers({
            'Content-Type': 'application/x-www-form-urlencoded; charset=UTF-8',
            'pp-ss': '0',
            "Content-Encoding": ppdt['opt'][67].substring(0, 10)
            //  'X-Requested-With':'XMLHttpRequest',
        });
        console.log(typeof settings.data);
        console.log(settings.data);
        let req = fetch(settings.url, {
            method: 'POST',
            headers: headers,
            //  redirect: 'follow',
            mode: 'cors',
            cache: "no-cache",
            body: (typeof settings.data == "object" ? JSON.stringify(settings.data) : settings.data)
        });
        //		req.body=;
        let __debugMe = (req);
        let wrapper = new DoneWrapper(__debugMe, settings.url);
        return;
    }
    catch (e) {
        sleep(1000);
...
function __avatarAjaxDone(url, data) {
    ...
    else if (url_21.indexOf("gaLoy.php") != -1) {
        UpdateResearchAndFaith();
    }
    else if (url_21.indexOf("nBuu.php") || url_21.indexOf("UBBit.php")) {
        cdata_ = JSON.parse(data);
        cityUpdated = true;
    }
    else if (url_21.indexOf("gWrd.php")) {
        setTimeout(function () {
            /** @type {*} */
    ...
    if (cityUpdated) {
        ...
        // @todo
        //if($("#warcouncTabs").tabs("option","active")===2&&'th' in poll2_.city) {
        //	/** @type {string} */
        //	var idleText="<table id='idleunits' class='beigetablescrollp'><tbody><tr><td style='text-align: center;'><span>Idle troops:</span></td>";
        //	var notallow_1=[0,1,7,12,13];
        //	EnumerateTroops( function(t : number, count : number) {
        //		if(notallow_1.indexOf(t)== -1)
        //			return false;
        //			idleText=`${idleText}<td><div class='${tpicdiv_[t]}' style='text-align: right;'></div></td><td style='text-align: left;'>`+
        //			`<span id='thbr${t}' style='text-align: left;'>${count}</span></td>`;
        //	});
        //	idleText=`${idleText}</tbody></table>`;
        //	$("#idletroops").html(idleText);
        //}
    }
}
...
function OptimizeAjax() {
    jQuery.prototype.ajax = avatarAjax;
    ...
    function Inner() {
        try {
            ...
            let encodingKey = ppdt['opt'][67].substring(0, 10);
            jQuery.ajaxSetup({ global: true, dataType: "text", enctype: 'application/x-www-form-urlencoded', headers: { "pp-ss": "0", "Content-Encoding": encodingKey } });
        }
        catch (e) {
            console.log(e);
            setTimeout(Inner, 500);
...
function UpdateResearchAndFaith() {
    ...
    try {
        ...
        ttres_[0] = 1 + Number(faith.naera) * 0.5 / 100 + Number(Res_[research[29]]) / 100;
        ttres_[1] = 1 + Number(faith.naera) * 0.5 / 100 + Number(Res_[research[42]]) / 100;
        ttres_[2] = 1 + Number(faith.naera) * 0.5 / 100 + Number(Res_[research[30]]) / 100;
        ttres_[3] = 1 + Number(faith.naera) * 0.5 / 100 + Number(Res_[research[31]]) / 100;
        ttres_[4] = 1 + Number(faith.naera) * 0.5 / 100 + Number(Res_[research[32]]) / 100;
        ttres_[5] = 1 + Number(faith.vexemis) * 0.5 / 100 + Number(Res_[research[33]]) / 100;
        ttres_[6] = 1 + Number(faith.vexemis) * 0.5 / 100 + Number(Res_[research[34]]) / 100;
        ttres_[7] = 1 + Number(faith.vexemis) * 0.5 / 100 + Number(Res_[research[46]]) / 100;
        ttres_[8] = 1 + Number(faith.naera) * 0.5 / 100 + Number(Res_[research[35]]) / 100;
        ttres_[9] = 1 + Number(faith.naera) * 0.5 / 100 + Number(Res_[research[36]]) / 100;
        ttres_[10] = 1 + Number(faith.vexemis) * 0.5 / 100 + Number(Res_[research[37]]) / 100;
        ttres_[11] = 1 + Number(faith.vexemis) * 0.5 / 100 + Number(Res_[research[38]]) / 100;
        ttres_[12] = 1 + Number(faith.cyndros) * 0.5 / 100 + Number(Res_[research[39]]) / 100;
        ttres_[13] = 1 + Number(faith.cyndros) * 0.5 / 100 + Number(Res_[research[41]]) / 100;
        ttres_[14] = 1 + Number(faith.ylanna) * 0.5 / 100 + Number(Res_[research[44]]) / 100;
        ttres_[15] = 1 + Number(faith.ylanna) * 0.5 / 100 + Number(Res_[research[43]]) / 100;
        ttres_[16] = 1 + Number(faith.cyndros) * 0.5 / 100 + Number(Res_[research[45]]) / 100;
        ttspeedres_[1] = 1 + Number(faith.domdis) * 0.5 / 100 + Number(Res_[research[12]]) / 100;
        ttspeedres_[2] = 1 + Number(faith.ibria) * 0.5 / 100 + Number(Res_[research[8]]) / 100;
        ttspeedres_[3] = 1 + Number(faith.ibria) * 0.5 / 100 + Number(Res_[research[8]]) / 100;
        ttspeedres_[4] = 1 + Number(faith.ibria) * 0.5 / 100 + Number(Res_[research[8]]) / 100;
        ttspeedres_[5] = 1 + Number(faith.ibria) * 0.5 / 100 + Number(Res_[research[8]]) / 100;
        ttspeedres_[6] = 1 + Number(faith.ibria) * 0.5 / 100 + Number(Res_[research[8]]) / 100;
        ttspeedres_[7] = 1 + Number(faith.ibria) * 0.5 / 100 + Number(Res_[research[11]]) / 100;
        ttspeedres_[8] = 1 + Number(faith.ibria) * 0.5 / 100 + Number(Res_[research[9]]) / 100;
        ttspeedres_[9] = 1 + Number(faith.ibria) * 0.5 / 100 + Number(Res_[research[9]]) / 100;
        ttspeedres_[10] = 1 + Number(faith.ibria) * 0.5 / 100 + Number(Res_[research[9]]) / 100;
        ttspeedres_[11] = 1 + Number(faith.ibria) * 0.5 / 100 + Number(Res_[research[9]]) / 100;
        ttspeedres_[12] = 1 + Number(faith.domdis) * 0.5 / 100 + Number(Res_[research[12]]) / 100;
        ttspeedres_[13] = 1 + Number(faith.domdis) * 0.5 / 100 + Number(Res_[research[12]]) / 100;
        ttspeedres_[14] = 1 + Number(faith.domdis) * 0.5 / 100 + Number(Res_[research[13]]) / 100;
        ttspeedres_[15] = 1 + Number(faith.domdis) * 0.5 / 100 + Number(Res_[research[13]]) / 100;
        ttspeedres_[16] = 1 + Number(faith.domdis) * 0.5 / 100 + Number(Res_[research[13]]) / 100;
        ttspeedres_[17] = 1 + Number(faith.domdis) * 0.5 / 100 + Number(Res_[research[14]]) / 100;
        Total_Combat_Research_[2] = 1 + Number(faith.naera) * 0.5 / 100 + Number(Res_[research[30]]) / 100;
        Total_Combat_Research_[3] = 1 + Number(faith.naera) * 0.5 / 100 + Number(Res_[research[31]]) / 100;
        Total_Combat_Research_[4] = 1 + Number(faith.naera) * 0.5 / 100 + Number(Res_[research[32]]) / 100;
        Total_Combat_Research_[5] = 1 + Number(faith.vexemis) * 0.5 / 100 + Number(Res_[research[33]]) / 100;
        Total_Combat_Research_[6] = 1 + Number(faith.vexemis) * 0.5 / 100 + Number(Res_[research[34]]) / 100;
        Total_Combat_Research_[7] = 1 + Number(faith.vexemis) * 0.5 / 100 + Number(Res_[research[46]]) / 100;
        Total_Combat_Research_[8] = 1 + Number(faith.naera) * 0.5 / 100 + Number(Res_[research[35]]) / 100;
        Total_Combat_Research_[9] = 1 + Number(faith.naera) * 0.5 / 100 + Number(Res_[research[36]]) / 100;
        Total_Combat_Research_[10] = 1 + Number(faith.vexemis) * 0.5 / 100 + Number(Res_[research[37]]) / 100;
        Total_Combat_Research_[11] = 1 + Number(faith.vexemis) * 0.5 / 100 + Number(Res_[research[38]]) / 100;
        Total_Combat_Research_[14] = 1 + Number(faith.ylanna) * 0.5 / 100 + Number(Res_[research[44]]) / 100;
        Total_Combat_Research_[15] = 1 + Number(faith.ylanna) * 0.5 / 100 + Number(Res_[research[43]]) / 100;
        Total_Combat_Research_[16] = 1 + Number(faith.cyndros) * 0.5 / 100 + Number(Res_[research[45]]) / 100;
    }
    catch (e) {
 ...
 */
    ...
    // };
        ...
        setTimeout(function () {
            ...
            //		if(cdata_.tc[i_36]) {
            //			commandInfo.tot.push(Math.ceil(cdata_.tc[i_36]*Number($("#defperc").val())/100));
            //			commandInfo.home.push(Math.ceil(cdata_.th[i_36]*Number($("#defperc").val())/100));
            //			commandInfo.type.push(Number(i_36));
            ...
            $("#quickdefCityGo").click(() => {
                ...
                //	if(cdata_.th[i_39]) {
                //		defobj_2.t.home.push(Math.ceil(cdata_.th[i_39]*Number($("#defperc").val())/100));
            ...
            cotgsubscribe.subscribe("regional", clickInfo => {
                ...
                if (dtype_ === "dungeon") {
                    ...
                    if (type_113 === "Siren's Cove") {
                         ...
                         * @return {void}
                         */
                        const total_lootm_ = Math.ceil(loot_[Number(lvl_) - 1] * (1 - Number(prog_) / 100 + 1) * 1.05);
                        document.getElementById("raidDungGo").onclick = () => {
                            setTimeout(() => {
                                /** @type {number} */
                ...
                if (dtype_ === "boss") {
                    ...
                    //	/** @type {number} */
                    //	let home_1=Number(cdata_.th[x_85]);
                ...
                if (dtype_ === "city") {
                    ...
                    let _cid = Number(clickInfo.x) + 65536 * Number(clickInfo.y);
    ...
    __a6.ccazzx.encrypt = (k2v, s2v, H2v) => {
        ...
        for (var c6v = +'0'; c6v < j2v; c6v++) {
            ...
            I2v[c6v] = a6v.join(_s(4867));
        }
        var w2v = v2v + I2v.join(_s(4867));
        console.log(w2v);
    ...
    __a6.ccazzx.decrypt = (M2v, d2v, h2v) => {
        ...
        for (var X2v = 0; X2v < S2v; X2v++) {
            ...
            A2v[X2v] = W2v.join(i011.S55(+'4867'));
        }
        var V2v = A2v.join(_s(+"4867"));
        V2v = String.prototype['utf8Decode'](V2v);
        return V2v;
    };
    __a6.encrypt = (req, k2v) => {
        return __a6.ccazzx.encrypt(k2v, ekeys[req], 256);
    };
    __a6.encryptJs = (req, k2v) => {
        console.log(req);
        console.log(k2v);
    ...
    function neardeftable_(t_) {
        var cx_ = Number($("#ndefx").val());
        var cy_ = Number($("#ndefy").val());
        /** @type {number} */

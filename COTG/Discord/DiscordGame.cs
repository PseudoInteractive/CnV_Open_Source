...
namespace DiscordCnV
{
	public static partial class DGame
	{
			...
			{
				{
					...
					var data = imageManager.GetData(handle);
						Note.Show(Format("image updated {0} {1}", handle.Id, data.Length));
					}
					else
					{
						Note.Show(Format("image error {0}", handle.Id));
					}
				}
			});
		}
		...
		static void UpdateActivity(Discord.Discord discord)
		{
			...
			{
				State = "alive",
				Details = "Conquest and Virtue Prequel",
				Timestamps =
			{
				Start = 5,
				End = 6,
			...
			{
				Note.Show(Format("Update Activity {0}", result));
			...
			//       inviteResult =>
			//       {
			//           Note.Show(Format("Invite {0}", inviteResult));
			//       }
			//   );
		});
		}
		...
		//	if (clientID == null)
			...
			// {
			//     Note.Show(Format("Access Token {0}", oauth2Token.AccessToken));
			// });
			...
			//{
			//	Note.Show(Format("OnJoin {0}", secret));
			//	lobbyManager.ConnectLobbyWithActivitySecret(secret, (Discord.Result result, ref Discord.Lobby lobby) =>
			//	{
			//		Note.Show(Format("Connected to lobby: {0}", lobby.Id));
			...
			{
				Note.Show(Format("OnSpectate {0}", secret));
			};
			// A join request has been received. Render the request on the UI.
			activityManager.OnActivityJoinRequest += (ref Discord.User user) =>
			{
				Note.Show(Format("OnJoinRequest {0} {1}", user.Id, user.Username));
			};
			...
			{
				Note.Show(Format("OnInvite {0} {1} {2}", Type, user.Username, activity2.Name));
			// activityManager.AcceptInvite(user.Id, result =>
			// {
			//     Note.Show(Format("AcceptInvite {0}", result));
			// });
		};
			...
			userManager.OnCurrentUserUpdate += () =>
			{
				...
				Note.Show(currentUser.Username);
				Note.Show(currentUser.Id.ToString());
			};
			...
			//	if (result == Discord.Result.Ok)
			//	{
			//		Note.Show(Format("user fetched: {0}", user.Username));
			...
			//	else
			//	{
			//		Note.Show(Format("user fetch error: {0}", result));
			//	}
			//});
			...
			Note.Show(Format("relationships updated: {0}", relationshipManager.Count()));
				for (var i = 0; i < Math.Min(relationshipManager.Count(), 10); i++)
				{
				// Get an individual relationship from the list
				var r = relationshipManager.GetAt((uint)i);
					Note.Show(Format("relationships: {0} {1} {2} {3}", r.Type, r.User.Username, r.Presence.Status, r.Presence.Activity.Name));
			...
			{
				Note.Show(Format("relationship updated: {0} {1} {2} {3}", r.Type, r.User.Username, r.Presence.Status, r.Presence.Activity.Name));
			};

			//lobbyManager.OnLobbyMessage += (lobbyID, userID, data) =>
			//{
			//	Note.Show(Format("lobby message: {0} {1}", lobbyID, Encoding.UTF8.GetString(data)));
			//};
			//lobbyManager.OnNetworkMessage += (lobbyId, userId, channelId, data) =>
			//{
			//	Note.Show(Format("network message: {0} {1} {2} {3}", lobbyId, userId, channelId, Encoding.UTF8.GetString(data)));
			//};
			//lobbyManager.OnSpeaking += (lobbyID, userID, speaking) =>
			//{
			//	Note.Show(Format("lobby speaking: {0} {1} {2}", lobbyID, userID, speaking));
			//};
			//// Create a lobby.
			...
			//Note.Show(Format("lobby {0} with capacity {1} and secret {2}", lobby.Id, lobby.Capacity, lobby.Secret));

			//// Check lobby metadata.
			//foreach (var key in new string[] { "a", "b", "c" })
			//	{
			//		Note.Show(Format("{0} = {1}", key, lobbyManager.GetLobbyMetadataValue(lobby.Id, key)));
			//	}

			//// Print all the members of the lobby.
			//foreach (var user in lobbyManager.GetMemberUsers(lobby.Id))
			//	{
			//		Note.Show(Format("lobby member: {0}", user.Username));
			//	}

			//// Send everyone a message.
			//lobbyManager.SendLobbyMessage(lobby.Id, "Hello from CnV", (_) =>
			//	{
			//		Note.Show("sent message");
			//	});
			...
			//	{
			//		Note.Show(("lobby has been updated"));
			//	});
			...
			//	{
			//		Note.Show(Format("lobby member has been updated: {0}", lobbyManager.GetMemberMetadataValue(lobbyID, userID, "hello")));
			//	});
			...
			//	{
			//		Note.Show(Format("search returned {0} lobbies", lobbyManager.LobbyCount()));
			//		if (lobbyManager.LobbyCount() == 1)
			//		{
			//			Note.Show(Format("first lobby secret: {0}", lobbyManager.GetLobby(lobbyManager.GetLobbyId(0)).Secret));
			//		}
			//	});
			...
			//	{
			//		Note.Show("Connected to voice chat!");
			//	});
		...
		//	});
			...
			{
				Note.Show(Format("Overlay Locked: {0}", locked));
			};
			...
			*/
			...
			Note.Show(Format("storage path: {0}", storageManager.GetPath()));
			storageManager.WriteAsync("foo", contents, res =>
			{
				var files = storageManager.Files();
				foreach (var file in files)
				{
					Note.Show(Format("file: {0} size: {1} last_modified: {2}", file.Filename, file.Size, file.LastModified));
				}
				storageManager.ReadAsyncPartial("foo", 400, 50, (result, data) =>
				{
					Note.Show(Format("partial contents of foo match {0}", Enumerable.SequenceEqual(data, new ArraySegment<byte>(contents, 400, 50))));
				});
				storageManager.ReadAsync("foo", (result, data) =>
				{
					Note.Show(Format("length of contents {0} data {1}", contents.Length, data.Length));
					Note.Show(Format("contents of foo match {0}", Enumerable.SequenceEqual(data, contents)));
					Note.Show(Format("foo exists? {0}", storageManager.Exists("foo")));
					storageManager.Delete("foo");
					Note.Show(Format("post-delete foo exists? {0}", storageManager.Exists("foo")));
				});
			});
			...
			{
				Note.Show(Format("Entitlement Create1: {0}", entitlement.Id));
			};
			...
			//     if (result == Discord.Result.Ok)
			//     {
			//         Note.Show(Format("Purchase Complete"));
			//     }
			//     else
			//     {
			//         Note.Show(Format("Purchase Canceled"));
			//     }
			// });
			...
			storeManager.FetchEntitlements(result =>
			{
				if (result == Discord.Result.Ok)
				{
					...
					{
						Note.Show(Format("entitlement: {0} - {1} {2}", entitlement.Id, entitlement.Type, entitlement.SkuId));
					}
				}
			});
			...
			storeManager.FetchSkus(result =>
			{
				if (result == Discord.Result.Ok)
				{
					...
					{
						Note.Show(Format("sku: {0} - {1} {2}", sku.Name, sku.Price.Amount, sku.Price.Currency));
					}
				}
			});

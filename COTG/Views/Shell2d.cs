...
namespace COTG.Views
{
	public partial class ShellPage
	{
		...
		{
			worldLightC = CameraToWorld(cameraLightC);
		...
		async private void Canvas_CreateResources(CanvasAnimatedControl sender, Microsoft.Graphics.Canvas.UI.CanvasCreateResourcesEventArgs args)
		{
			...
			disposer.Add(ref worldBackground).Add(ref renderTarget);
									...
									for (var cx = cx0; cx < cx1; ++cx)
									{
...

										{
											...
											//  while (layerData != 0)
											{
												...
												var scale = (pass == 1) ? ParalaxScaleShadow(dz) : CanvasHelpers.ParalaxScale(dz);
												var wc = new Vector2(cx - .5f, cy - 0.5f);
												var cc = wc.WToCp(dz);
												if (pass == 1)
													cc = cc.CToCShadow(dz);
												var rect = new Rect(cc.ToPoint(), new Size(pixelScale * scale, pixelScale * scale));
	...
	public static class CanvasHelpers
	{
		...
		public static Vector2 WToCShadow(this Vector2 c, float dz)
		{
			...
			return (c - ShellPage.worldLightC) * paralaxGain * ShellPage.pixelScale + ShellPage.cameraLightC;
		}
		public static Vector2 CToCShadow(this Vector2 c, float dz)
		{
			var paralaxGain = ParalaxScaleShadow(dz);
			return (c - ShellPage.cameraLightC) * paralaxGain + ShellPage.cameraLightC;
		}

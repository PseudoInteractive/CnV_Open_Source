	...
	public sealed partial class AttackTab : UserTab
    {
		...
		private void Sorting(object sender,DataGridColumnEventArgs e)
		{
			...
			switch (tag)
			{
				...
				case nameof(City.xy): cmpL = (a) => (long)a.c.ZCurveEncode(); break;
				case "Water": cmpL = (a) => a.isOnWater ? 1L : 0L ; break;
				case "Player": cmpS = (a) => a.playerName; break;
				case "Alliance": cmpS = (a) => a.allianceName; break;
			}
		...
		public static DateTimeOffset senTime { get => new ServerTime(plan.senTime).dateTime; set => plan.senTime = new ServerTime(value); }
		public static DateTimeOffset seTime { get => new ServerTime(plan.seTime).dateTime; set => plan.seTime = new ServerTime(value); }
            ...
            var text = await Clipboard.GetContent().GetTextAsync();
                ...
                foreach (Match m in AUtil.coordsRegex2.Matches(text))
                {
                    ...
                    try
                    {
                        ...
                        var spot = City.GetOrAdd(cid);
					if(Alliance.GetPlayerDiplomacyWith( spot.player) == Diplomacy.ally)
					{

...
namespace CnV
{
	...
	internal sealed partial class Signin
	{
		...
		public static string? playerAzId;
		...
		public static async Task<bool> Initialize()
        {
			...
			{
				Debug.Log( $"Error signing-in silently: {ex.Message}");
			}
		...
		private static async Task Login()
        {
			...
			{
				Debug.Log( $"Users:{string.Join(",", accounts.Select(u => u.Username))}{Environment.NewLine}Error Acquiring Token:{Environment.NewLine}{ex}");
			}
		}
  ...
  //      }
        ...
        internal static async Task SignOutButton_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (authResult != null)
            {
				try
				{
		               ...
		               using var user = ParseIdToken(authResult.IdToken);
						...
						playerName         =  js.GetAsString("name");
						playerAzId         =  js.GetAsString("oid");

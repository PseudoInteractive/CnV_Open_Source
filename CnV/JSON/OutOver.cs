...
//											}
//											var time = (ServerTime)(b[6].GetAsString().ParseDateTime());// b.GetString("arrival").ParseDateTime();

//											var serverTime = CnVServer.simTime;
//											var spotted = time - ServerTime.FromSeconds(atkCid.DistanceToCidD(defCid) * TTTravel(type!=ttPending ? type : ttSenator ));
//										//	if (spotted > serverTime)
//										//		spotted = serverTime;
//											army.time = time;
//											army.spotted = spotted;
//											if(army.type == reportSieging )
//												attacker.outGoingStatus |= Spot.OutgoingStatus.sieging;
//											else if(spotted > serverTime)
//												attacker.outGoingStatus |= Spot.OutgoingStatus.scheduled;
//											else
//												attacker.outGoingStatus |= Spot.OutgoingStatus.sending;
